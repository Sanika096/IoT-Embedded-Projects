...
const int output26 = 42;
const int output27 = 5;
const int output28 = 6;
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print("Check your Hotspot & try again!..\n");
  }
...
void loop(){
  ...
  if (client) {                             // If a new client connects,
    ...
    Serial.println("Sanika.");          // print a message out in the serial port
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            // Web Page Heading
            client.println("<body><h1>Sanika's Smart Home</h1>");
